...
def run_app():
    ...
    try:
        for conn in psutil.net_connections(kind='inet'):
            if conn.laddr.port == SERVER_CONFIG['chisel_port'] and conn.status == 'LISTEN':
                print(colored(f"Another instance is already running on port {SERVER_CONFIG['chisel_port']} (PID: {conn.pid})", "yellow"))
                return
    except Exception as e:
    ...
    try:
        ...
        uvicorn.run(
            "main:app",
            host="0.0.0.0",
            port=SERVER_CONFIG['chisel_port'],
